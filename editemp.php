...
function put_desig_combo($cmbname,$sel_desig="XXX")
{
	...
	//if($conn1=pg_connect("dbname=financetest host=localhost port=5432 user=postgres password=1234"));
        else die(pg_error());
...
function put_office_combo($cmbname,$sel_office="XXX")
{
	...
	//if($conn2=pg_connect("dbname=financetest host=localhost port=5432 user=postgres password=1234"));
        else die(pg_error());
	$retcmb="<select name=\"$cmbname\">";
	//selecting the designations
	//************************************************************
...
function put_scale_combo($cmbname,$sel_scale="XXX")
{
	...
	//if($conn=pg_connect("dbname=financetest host=localhost port=5432 user=postgres password=1234"));
        else die(pg_error());
	$retcmb="<select name=\"$cmbname\">";
	//selecting the scales
	//************************************************************
	$qry = "select scale,pay_scale.id from pay_scale,pay_master where pay_master.id = pay_scale.payid and pay_master.stype='S' and pay_scale.payid in (select payid from pay_scale where pay_scale.payid>=6);";
