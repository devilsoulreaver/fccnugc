<?php
 ...
 if($_POST){
	...
	$empid			= $_POST['empid'];
	$gross_salary	= ($_POST['gross_salary'])?$_POST['gross_salary']:0;
	...
	$less_allowance	= ($_POST['less_allowance'])?$_POST['less_allowance']:0;
	...
	$tax_employment	= ($_POST['tax_employment'])?$_POST['tax_employment']:0;
	
	if(!$empid || strlen($_POST['gross_salary'])==0 ){
		$errmsg="Required Information Is Incomplete";
	/*}elseif(!is_numeric($empid) || !is_numeric($gross_salary) ){
		...
		$empdetails1	= $form16Obj->getEmployeeDesignation($empid);
		//print_r($empdetails1);
		if(!$empdetails){
			...
			if($prof_taxs){
				foreach($prof_taxs as $prof_tax){
					$tax_employment1	+= $prof_tax['amount'];
				}
			}
			$tax_employment = $tax_employment1 + $tax_employment;
			...
			if($med_gross){
				$med_amt = ($med_gross>15000)?15000:$med_gross;
			}
			if($maintain_gross){
			...
			if($med_gross){
				$qualifing_amt	= $qualifing_amt + 15000;
			}
			if($maintain_gross){
			...
			if($total_income < $taxlimit){
				...
				if(($total_income > 300000) and ($total_income < 500001)){
					...
					$tax_payable	= $tx1 + ($tax_calc_amt * .2);
				}
				if($total_income > 500000){
					$tax_calc_amt   = $total_income - 500000;
					$tax_payable	= $tx2 + ($tax_calc_amt * .3);
				}
