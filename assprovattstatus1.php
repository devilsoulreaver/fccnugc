...
{	
	...
	$sql="select coalesce(count(distinct a.empid),0) from emp_master a,emp_transfer b where b.officeid='$officeid' and empcategoryid(a.empid)<>'G' and length(a.empid)>4  and empdesig(a.empid) = 'Assistant (Provisional)' and a.doret>current_date and got_attendence(a.empid,'$m1')='Y' and b.enddate='01/01/1900' and a.empid=b.empid";
	$trec1=pg_exec($conn,$sql);

	$sql="select count(distinct a.empid) from emp_master a,emp_transfer b where b.officeid='$officeid' and empcategoryid(a.empid)<>'G' and length(a.empid)>4  and empdesig(a.empid)= 'Assistant (Provisional)' and a.doret>current_date and a.empid=b.empid and b.enddate='01/01/1900'";
	...
	if ($totno<>$gotno) 
	{
	print "<td><a href=getpendingatt1.php?office=$officeid&ofdate=$m1 target=mainFrame>Details</a></td></tr>";
	}
	else
	{
		print "<td>.</td></tr>";
