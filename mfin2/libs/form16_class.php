<?php
	class form16{
		...
		function getTaxDeducted($ind_bill_nos){
			$sql	= "SELECT * FROM paybill where ind_bill_no in($ind_bill_nos) and effectdate < '2010-04-01' and effectdate > '2009-05-31' and indid='INCTAX' ORDER BY effectdate ASC";
			$result = pg_query($sql);
			if(pg_num_rows($result) > 0){ 
				$i =0;
				while($data = pg_fetch_object($result)) {
